    ...
    try:
        ...
        #第二志愿被分配
            ...
            for rs in rss:
                ...
                sql = '''SELECT a.volunteer_topic_no_id FROM app_application a
                                         LEFT JOIN app_topic b ON b.id=a.volunteer_topic_no_id
                                         WHERE a.volunteer_no=2 AND b.tea_no = ''' + str(
                ...
                for rst in rsts:
                    ...
                    sql = '''SELECT stu_no,score FROM
                                             (SELECT stu_no,volunteer_topic_no_id,score FROM app_application a 
                                             LEFT JOIN app_select_sysuser b ON b.user_id = a.stu_no 
                                             WHERE volunteer_no = 2 AND volunteer_topic_no_id = ''' + str(
                        volunteer_topic_no_id) + ''' 
                                             ORDER BY score DESC limit 10000000000) T1 
                                             GROUP BY volunteer_topic_no_id'''
                    rs_ss = fetch_local(sql, [])
                    score = rs_ss[0]['score']
            ...
            for rs in rss:
                ...
                for rst in rsts:
                    ...
                    sql = '''SELECT stu_no,score FROM
                                             (SELECT stu_no,volunteer_topic_no_id,score FROM app_application a 
                                             LEFT JOIN app_select_sysuser b ON b.user_id = a.stu_no 
                                             WHERE volunteer_no = 3 AND volunteer_topic_no_id = ''' + str(
                        volunteer_topic_no_id) + ''' 
                                             ORDER BY score DESC limit 10000000000) T1 
                                             GROUP BY volunteer_topic_no_id'''
                    rs_ss = fetch_local(sql, [])
            ...
            for rs in rss:
                ...
                for rst in rsts:
                    ...
                    sql = '''SELECT stu_no,score FROM
                                             (SELECT stu_no,volunteer_topic_no_id,score FROM app_application a 
                                             LEFT JOIN app_select_sysuser b ON b.user_id = a.stu_no 
                                             WHERE volunteer_no = 4 AND volunteer_topic_no_id = ''' + str(
                        volunteer_topic_no_id) + ''' 
                                             ORDER BY score DESC limit 10000000000) T1 
                                             GROUP BY volunteer_topic_no_id'''
                    rs_ss = fetch_local(sql, [])
        ...
        # 第五志愿被分配
            ...
            for rs in rss:
                ...
                for rst in rsts:
                    ...
                    sql = '''SELECT stu_no,score FROM
                                             (SELECT stu_no,volunteer_topic_no_id,score FROM app_application a 
                                             LEFT JOIN app_select_sysuser b ON b.user_id = a.stu_no 
                                             WHERE volunteer_no = 5 AND volunteer_topic_no_id = ''' + str(
                        volunteer_topic_no_id) + ''' 
                                             ORDER BY score DESC limit 10000000000) T1 
                                             GROUP BY volunteer_topic_no_id'''
                    rs_ss = fetch_local(sql, [])
            ...
            print '第5志愿被分配'
        # 第六志愿被分配
        elif time.time() >= (start_time+16.5*60*60.0) and time.time() < (start_time+17.5*60*60.0):
            # 找到本志愿阶段其志愿被选到的所有教师
            sql = '''SELECT a.volunteer_topic_no_id,b.tea_no FROM app_application a 
                                     LEFT JOIN app_topic b ON b.id = a.volunteer_topic_no_id            
            ...
            for rs in rss:
                ...
                for rst in rsts:
                    ...
                    sql = '''SELECT stu_no,score FROM
                                             (SELECT stu_no,volunteer_topic_no_id,score FROM app_application a 
                                             LEFT JOIN app_select_sysuser b ON b.user_id = a.stu_no 
                                             WHERE volunteer_no = 6 AND volunteer_topic_no_id = ''' + str(
                        volunteer_topic_no_id) + ''' 
                                             ORDER BY score DESC limit 10000000000) T1 
                                             GROUP BY volunteer_topic_no_id'''
                    rs_ss = fetch_local(sql, [])
            ...
            print '第6志愿被分配'
        #自由分配start
        elif time.time() >= (start_time+17.5*60*60.0) and time.time() < (start_time+18.5*60*60.0):
            # 自由分配start
